...
export default async function CoachDetailPage({
  ...
  let availability: CoachAvailabilitySlot[] = [];
  let availabilityError: string | null = null;

  try {
    const availRes = await apiFetch<ApiWrapper<CoachAvailabilitySlot[]>>(
      `/coaches/${id}/availability`
    );
    availability = availRes.data ?? [];
  } catch (e) {
    // A 404 here would mean the profile stopped being ACTIVE between the two
    // requests — treat it the same as "no slots" rather than as an error.
    if (!(e instanceof ApiError && e.status === 404)) {
      const msg = e instanceof Error ? e.message : "";
      availabilityError = msg || "Không thể tải lịch rảnh của huấn luyện viên.";
    }
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <CoachDetailInteractive
        coachId={coach.id}
        coachDisplayName={coach.displayName}
        sports={coach.sports}
        hourlyRateLabel={formatHourlyRate(coach.hourlyRate)}
