...
namespace MATCHOP.API.Controllers
{
    ...
    public class ProfileController : ControllerBase
    {
        ...
        public async Task<IActionResult> UploadAvatar([FromForm] UploadAvatarRequestDto dto)
        {
            var userId = GetUserId();
            var avatarUrl = await _userService.UploadAvatarAsync(userId, dto);
        ...
        {
            var userId = GetUserId();
            var result = await _userService.GetFavoriteSportsAsync(userId);
            return Ok(ApiResponse<List<FavoriteSportDto>>.Ok(result));
        }

        [HttpPut("favorite-sports")]
        public async Task<IActionResult> UpdateFavoriteSports(UpdateFavoriteSportsRequestDto dto)
        {
            var userId = GetUserId();
            var result = await _userService.UpdateFavoriteSportsAsync(userId, dto);
